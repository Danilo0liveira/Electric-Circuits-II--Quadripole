...
def reatores():
        ...
        for c in zls:
            ...
            # network.set_resultant_matrix(network.cascade_connection(network.resultant_mtrx, reator1.net_mtrx))

            t1 = Transformer(n= 3.33, z1= 7.6e-3 + 3.8e-3j, z2= 33.9e-3 + 0.85e-3j, y=(4320 + 5050j)/(4320*5050j))
            ...
            t2 = Transformer(n= 0.6, z1= 7.6e-3 + 3.8e-3j, z2= 33.9e-3 + 0.85e-3j,
            ...
            t3=Transformer(n=69, z1= 7.6e-3 + 3.8e-3j, z2= 33.9e-3 + 0.85e-3j,
            ...
            reator3 = ShuntAdmittance(z=1j*c)
            
            network.set_resultant_matrix(network.cascade_connection(network.resultant_mtrx, reator3.net_mtrx))
            network.set_resultant_matrix(network.cascade_connection(network.resultant_mtrx, reator3.net_mtrx))
            ...
            # # reator2 = ShuntAdmittance(z=1j*c)
            # net2.set_resultant_matrix(network.cascade_connection(t3.net_mtrx, lt6.net_mtrx))
            ...
            # # reator1 = ShuntAdmittance(z=1j*c)
            ...
            with open('dadosn1n2n3.txt','at') as f:
                # f.write(f'n1:{9.8}, Tensao1: {fasorvz1[0]}, {fasorvz1[1]}\n')
                # f.write(f'n2:{c}, Tensao2: {fasorvz2[0]}, {fasorvz2[1]}\n')
                f.write(f'n3:{c}, Tensao3: {fasorvz3[0]}, {fasorvz3[1]}\n')
                f.write('-='*32)
                f.write('\n')
